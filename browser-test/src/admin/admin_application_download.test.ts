...
test.describe('csv json pdf download test- two applications', () => {
  ...
  test('csv json pdf download test', async ({
    page,
    adminQuestions,
    adminPrograms,
    applicantQuestions,
    ...
    await test.step('Admin - Verify updated CSV and JSON exports', async () => {
      ...
      expect(
        postEditJsonContent[0].application.numbercsvdownload.number,
      ).toEqual(1500)
    })
    ...
    // #######################################
    await test.step('Test filter application without clicking button', async () => {
      await loginAsProgramAdmin(page)
      await adminPrograms.viewApplications(programName)
